...
//     const walletAddress = 'FQuxesCpn3giUaxmwFr98ee9FVMSChUziyVKLYYrFKmW';
...
const baseApiUrl = 'https://api.shyft.to';
const maxParallelRequests = 50;  // Shyft's rate limit
const walletAddress = 'FQuxesCpn3giUaxmwFr98ee9FVMSChUziyVKLYYrFKmW';
const apiKey = 'VM6jV28HbK8gyBKL';
...
async function fetchAllTokensAndMetadata() {
    const nftList = [];
    let page = 1;
    let hasMoreNFTs = true;

    // Fetch all tokens for the user
    const response = await fetch(`${baseApiUrl}/sol/v1/wallet/all_tokens?network=mainnet-beta&wallet=${walletAddress}`);
    const data = await response.json();
    console.log(response.status)
    console.log(response.statusText)
    const addresses = data.result.map(token => token.address);

    async function fetchBatchMetadata(batchAddresses) {
        const requestOptions = {
            ...
            body: JSON.stringify({
                ...
                "token_addresses": batchAddresses,
                "refresh": false,
                "token_record": true
            })
        };
        const response = await fetch(`${baseApiUrl}/sol/v1/nft/read_selected`, requestOptions);
        const data = await response.json();
        return transformNFTData(data.result || []);
    }

    // Batch fetching of token metadata
    const metadataPromises = [];
    for (let i = 0; i < addresses.length; i += maxParallelRequests) {
        const batchAddresses = addresses.slice(i, i + maxParallelRequests);
        metadataPromises.push(fetchBatchMetadata(batchAddresses));
    }
    ...
    // Now fetch regular and compressed NFTs for the wallet
    while (hasMoreNFTs) {
        const requestPromises = [];

        // Fetch in batches of up to maxParallelRequests
        for (let i = 0; i < maxParallelRequests && hasMoreNFTs; i++) {
            requestPromises.push(fetchNFTsForPage('/sol/v2/nft/read_all', page));
            requestPromises.push(fetchNFTsForPage('/sol/v2/nft/compressed/read_all', page));
            page++;
        }

        // Resolve this batch of requests
        const results = await Promise.all(requestPromises);

        // If any page in the batch has zero results, stop further fetching
    ...
    // Save the final NFT list
    writeFileSync('nftData.json', JSON.stringify(nftList, null, 2), 'utf-8');
    console.log('Fetched NFTs:', nftList.length);
}
